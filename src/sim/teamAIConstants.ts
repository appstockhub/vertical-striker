...
/**
 * ★24周目サイクル④★ easy難易度のCPU(Team B)が「守備側」の時の追跡権人数。
 *
 * 実測 (autonomous-log サイクル④参照): 2枚の純追跡 (引力3.0・リーシュ免除) +
 * 密着チャレンジ1%/tick の組は、スクリプト人間の保持を中央値61tick (約1秒) で刈り取り、
 * Team Aの攻撃チェーン44本が44本とも敵陣に入る前に消滅した (c6シュート0の主因)。
 * 同条件のCPU保持は326tick生存する (人間側のプレスはスクリプト駆動で鈍いため) —
 * この非対称を難易度の言葉で埋める: **easyのCPU守備はプレスが1枚** (2枚目のカバーが
 * 居ない = パスコースが1本開く)。原作のeasy CPUが「囲むが飛び込まない」寄りの守備で
 * 人間の前進を許す感触の近似でもある。medium/hard は従来どおり2枚。
 * 人間チーム (Team A) の守備枚数は難易度に依らず2枚のまま (難易度=CPUの強さ、の意味論)。
 */
export const CHASE_RIGHT_HOLDERS_DEFENDING_CPU_EASY = 1;
 ...
 */
// テンポ変更(24周目サイクル①)で 1.5秒→5秒 (1/BALL_TEMPO倍) にしたが、
// ★24周目サイクル④で90(1.5秒)へ差し戻した★。5秒への機械的スケーリングは誤りだった:
//  - このヒステリシスが守る対象 (GKパンチ・こぼれ球への「瞬間的な接触」по数十tick) は
//    ボール速度のテンポ変更でほぼ伸びていない (接触はボールが足元にある瞬間の現象)
//  - 一方で代償は「保持を切り替えてから300tick連続保持するまで陣形が押し上がらない」
//    こと。人間側の保持チェーンは平均153tick (サイクル④実測) なので、**Team Aは試合を
//    通じて一度もライン押し上げを得られず、キャリアが常に単騎で10人のブロックへ突入**
//    していた (c6シュート0の根本原因。スナップショット目視で確認: A保持で中盤へ運んでも
//    A野手10人がキックオフ隊形のまま静止)。CPU Bは326tickの長い保持で押し上げを享受
//    しており、非対称の自己強化になっていた。
// 90tick=1.5秒は「瞬間接触(数十tick)では切り替わらない」ガードを保ちつつ、
// 実在する保持チェーン(150tick+)なら押し上げが発動する境界。
...
/**
 * ★24周目サイクル④: レストオフェンス (残しFW)★
 * サポートランナー枠 (isSupportRunner、前線3スロット) が**守備時にも**維持する最低深度
 * (自ゴールからのdepth、px)。旧実装はライン引き下げで11人全員がボールと共に自陣へ
 * 沈むため、奪回位置が常に自陣ゴール前 → 攻撃のたびに1200px+の行軍が必要で、
 * 敵陣1/3への進入が試合を通じて0tickだった (c6の計測)。実サッカーの「カウンターの
 * 出し所として前線に残るFW」を、ホーム目標の下限クランプとして与える。
 * 880 = ハーフウェーライン。実際の位置は既存のオンサイドクランプが相手最終ラインの
 * 手前に頭打ちするため、相手が深く敷けば自動的にそのラインまで下がる (オフサイドには
 * ならない)。両チーム対称に適用する。
 */
export const REST_OFFENSE_MIN_DEPTH_FIXED: Fixed = toFixed(880);
