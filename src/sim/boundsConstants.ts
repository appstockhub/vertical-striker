...
/**
 * ★24周目サイクル②★ CPU側の再開キッカーが実際に蹴るまでの「間」(tick)。
 * 原作動画の実測 (parity-targets.md S1): GKの確保→配球まで約1.5秒静止 = 90tick。
 *
 * 導入の経緯 (重要): それまでCPUのセットプレー再開は「キッカーがボール脇に置かれ、
 * ホームへ歩き出した瞬間に旧ドリブルサーボがボールを吸着して1px動かし、ロックが
 * 解除される」という**サーボモデルの副作用**で偶然成立していた。離散タッチ化 (サイクル②)
 * でサーボが消えた途端、CPUの再開が構造的に不可能になった (コーナーで1030tickの
 * 安全網まで誰もボールを蹴らないのを実測)。再開は創発に頼らず明示的に蹴る。
 */
export const CPU_RESTART_DELAY_TICKS = 90;
