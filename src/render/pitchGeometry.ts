 ...
 * ★V-4で未使用化★ 縞の向きをY(奥行き)方向からX(幅)方向へ変更したため
 * (docs/visual-overhaul-proposal.md 1-2「原作は縦縞、自作は横縞で90度違う」への対応)、
 * 縞の間隔は STRIPE_WIDTH が担う。この定数自体は旧・真上視点実装 (pitchMarkings.ts、
 * 現在は未使用) が参照しているため残してある。
 */
export const STRIPE_DEPTH = 120;
/**
 * ★V-4★ 芝の刈り込み縞1本の幅 (ワールドpx、ピッチ幅方向)。
 * ゴール〜ゴールを結ぶ長辺方向に走る縦縞にするため、縞の分割をX軸に持たせる。
 * PITCH_WIDTH(480) を8等分する幅にして、原作画像で見える縞の本数感に合わせた。
 */
export const STRIPE_WIDTH = 60;
