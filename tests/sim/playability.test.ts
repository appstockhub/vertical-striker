...
describe('プレイアビリティ 0: キックが「押した時に必ず出る」', () => {
  ...
  /**
   * ★B連打の回帰テスト (24周目サイクル④)★ ワインドアップ導入(サイクル③)の副作用で、
   * B連打のたびに新しい解放が windupKick を上書きして ticksLeft がリセットされ、
   * **連打しているかぎりキックが永遠に発射されない**バグがあった (観戦シミュレーターの
   * aggressiveスクリプトがシュート圏内1400tick・B押下191回でシュート0本、が発見経路)。
   * 修正: ワインドアップ中は新しいチャージ/解放を受け付けない (モーションのコミット)。
   * 連打は実プレイで必ず起きる操作なので、因果ゲートとして常設する。
   */
  it('★B連打でもキックが発射される (ワインドアップが上書きで無限延期されない)', () => {
    let state = humanCarrying();
    let fired = false;
    // 2tick押し→2tick離しの高速連打を60tick続ける (ワインドアップ6tickより短い間隔)
    for (let i = 0; i < 60 && !fired; i++) {
      const held = i % 4 < 2;
      state = simulate(state, inputs(Direction8.Up, held ? { B: true } : {}));
      if (ballSpeed(state) > KICK_FIRED_THRESHOLD) fired = true;
    }
    expect(fired, 'B連打中にキックが一度も発射されない').toBe(true);
  });
